...
program hdf5_rw
  ...
  implicit none
  integer                        :: i, ii, status
  integer, parameter             :: num_links = 1000000, num_points = 10000
  ...
  ! per calcolo tempistiche:
  integer(i32)  :: dt_start(8), dt_end(8)
  real(r32)     :: start_time, finish_time, elapsed_time
  real(r32)     :: start_time_tot, finish_time_tot, elapsed_time_tot
  real(r32)     :: t1, t2, time, hours, minutes, seconds
  real(r32)     :: sph = 3600, ms = 60
...
!!! INIZIO LA LETTURA DEL FILE HDF5:
  ...
  time = t2-t1
  hours = time/sph
  minutes = mod(time, sph)/ms
  seconds = mod(minutes*ms, ms)

  write(output_unit, '(a)')  ''
  write(output_unit, '(a)'), ' -----------------  TIMING  ------------------'
  write(output_unit, '(a, i4, a1, i3, a1, f7.3, a)'), &
       ' CPU running time: ', int(hours), 'h',int(minutes), 'm', seconds, 's'

  start_time = real(dt_start(7))+real(dt_start(8))/1000_r32+real(dt_start(6))*ms+real(dt_start(5))*sph
  finish_time = real(dt_end(7))+real(dt_end(8))/1000_r32+real(dt_end(6))*ms+real(dt_end(5))*sph
  ...
  write(output_unit, '(a, i4, a1, i3, a1, f7.3, a)'), &
       ' wall-clock time:  ', int(hours), 'h', int(minutes), 'm', seconds, 's'
  write(output_unit, '(a)')  ''
