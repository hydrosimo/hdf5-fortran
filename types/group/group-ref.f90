...
PROGRAM main
  ...
  CHARACTER(LEN=16), PARAMETER :: filename   = "group-ref.h5"
  ...
  dim_r = 1
  dim_c = 10 ! ricordati di cambiare nel secondo ciclo DO
  ...
  call h5dcreate_f(group1, dsetname1, H5T_NATIVE_INTEGER, dataspace1, &
                 dset_id1, hdferr, crp_list)
  call h5dcreate_f(group1, dsetname2, H5T_NATIVE_REAL, dataspace2, &
                 dset_id2, hdferr, crp_list)
  call h5dcreate_f(group1, dsetname3, H5T_NATIVE_INTEGER, dataspace3, &
                 dset_id3, hdferr, crp_list)
  call h5dcreate_f(group2, dsetname4, H5T_STD_REF_DSETREG, dataspace4, &
  ...
  CALL h5sselect_elements_f(dataspace1, H5S_SELECT_SET_F, rank, num_points,&
                               coord, hdferr)
  CALL h5rcreate_f(group1, dsetname1, dataspace1, ref(1), hdferr)
  ...
  do ii=1,dim_r
    do jj=dim_c+1,2,-1
      ...
      CALL h5sselect_elements_f(dataspace1, H5S_SELECT_SET_F, rank, num_points,&
                                 coord, hdferr)
      CALL h5rcreate_f(group1, dsetname1, dataspace1, ref(1), hdferr)
      ! Extend dataset4 for references:
      ! uso 11 perchè con dim_c non funziona.
      size(1:2) = (/1,11-jj+2/)
      call h5dset_extent_f(dset_id4, size, hdferr)
      offset(1:2) = (/0,11-jj+1/)
      count(1:2)  = (/1,1/)
      call h5dget_space_f(dset_id4, dataspace4, hdferr)
      call h5sselect_hyperslab_f(dataspace4, H5S_SELECT_SET_F, &
          offset, count, hdferr)
      ! Write dataset4:
      ref_size(1) = 1
      CALL h5dwrite_f(dset_id4, H5T_STD_REF_DSETREG, ref, ref_size, hdferr, &
