...
PROGRAM H5_EXTEND
  ...
  CHARACTER(LEN=20), PARAMETER :: filename = "extend_1.h5"
  ...
  !data dimensions
  INTEGER(HSIZE_T), DIMENSION(1:2) :: dimsc
  ...
  do i = 1, 10
    ...
    CALL h5dwrite_f(dset_id, H5T_NATIVE_INTEGER, data2, data_dims, error, &
        memspace, dataspace)
    ! CALL h5dwrite_vl_f(dset_id, H5T_NATIVE_INTEGER, data2, data_dims, len, error, &
    !     memspace, dataspace)
  end do
