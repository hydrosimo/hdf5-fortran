...
! FILES
  ...
  CHARACTER(LEN=*), PARAMETER :: H5FILE_NAME = "nested.h5"
  CHARACTER(LEN=*), PARAMETER :: DATASETNAME = "array"

  INTEGER(KIND=int_k16), PARAMETER :: LENGTH = 10
  ! INTEGER(KIND=int_k16), PARAMETER :: LENGTH1 = 100000000
...
! Nested derived-type and dataset
  ...
  TYPE s1_t
     ...
     REAL(KIND=r_k8) :: b
     ! TYPE(s4_t) :: d
     INTEGER(int_k16), DIMENSION(1:4) :: d
     REAL(KIND=r_k8) :: c
  END TYPE s1_t
...
! Second derived-type (subset of s1_t) and dataset
  TYPE s2_t
     ...
     ! TYPE(s4_t) :: d
     INTEGER(int_k8), DIMENSION(1:4) :: d
  END TYPE s2_t

  TYPE(s2_t), TARGET, DIMENSION(1:length) :: s2
...
! Third "derived-type" (will be used to read float field of s1)
  ...
  ! -----------------------------------------------------------------------------------------------------------------
  ! ZERO STEP: SCRITTURA NEL DATASET
  ! -----------------------------------------------------------------------------------------------------------------
  ...
  ! -----------------------------------------------------------------------------------------------------------------
  ! PRIMO STEP: SCRITTURA NEL DATASET
  ! -----------------------------------------------------------------------------------------------------------------
  
  ! Initialize the data.
  ! Domanda: ma qui il ciclo do lo esegue direttamente su disco o su ram? Secondo me la chiamata su disco avviene quando
  ! chiamo la subroutine H5D_create_f
  DO i = 0, LENGTH-1
     ...
     s1(i+1)%d(1:4) = (/i,i*25,i*250,i*2500/)
  END DO
  
  ! -----------------------------------------------------------------------------------------------------------------
  ! SECONDO STEP: SCRITTURA NEL DATASET
  ! -----------------------------------------------------------------------------------------------------------------

  ! Create the data space: gestisce la struttura del dato, determina lo spazio e le sue dimensioni.
  CALL H5Screate_simple_f(RANK, dim, space, hdferr)

  ! Create the file: gestisce il file, lo crea, lo apre e lo chiude ecc.
  ...
  CALL H5Tcreate_f(H5T_COMPOUND_F, H5OFFSETOF(C_LOC(s1(1)), C_LOC(s1(2))), s1_tid, hdferr)
    ...
    CALL H5Tinsert_f(s1_tid, "chr", H5OFFSETOF(C_LOC(s1(1)),C_LOC(s1(1)%chr)),tid3, hdferr)
  CALL H5Tinsert_f(s1_tid, "a", H5OFFSETOF(C_LOC(s1(1)),C_LOC(s1(1)%a)), h5kind_to_type(int_k1,H5_INTEGER_KIND), hdferr)
  CALL H5Tinsert_f(s1_tid, "c", H5OFFSETOF(C_LOC(s1(1)),C_LOC(s1(1)%c)), h5kind_to_type(r_k8,H5_REAL_KIND), hdferr)
  CALL H5Tinsert_f(s1_tid, "b", H5OFFSETOF(C_LOC(s1(1)),C_LOC(s1(1)%b)), h5kind_to_type(r_k4,H5_REAL_KIND), hdferr)
    ! Create an array of integer datatype
    CALL H5Tarray_create_f(h5kind_to_type(int_k8,H5_INTEGER_KIND), 1, tdims1a, tid3a, hdferr)
    CALL H5Tinsert_f(s1_tid, "d", H5OFFSETOF(C_LOC(s1(1)),C_LOC(s1(1)%d)), tid3a, hdferr)
    ! CALL H5Tinsert_f(s1_tid, "d", H5OFFSETOF(C_LOC(s1(1)),C_LOC(s1(1)%d%x)), tid3a, hdferr)
  ...
  CALL H5Dclose_f(dataset, hdferr)   ! chiudo il datase, ma dopo lo riapro perchè devo copiarci alcuni valori da mettere nel type s2.
  CALL H5Sclose_f(space, hdferr)     ! chiudo anche lo spazio struttura perchè qui finisce la creazione del mio DB.

  ! -----------------------------------------------------------------------------------------------------------------
  ! TERZO STEP: LETTURA
  ! -----------------------------------------------------------------------------------------------------------------

  ! ! Open the file and the dataset: gestisco il file utilizzando la flag H5F_ACC_RDONLY_F = sola lettura)
  ! CALL H5Fopen_f(H5FILE_NAME, H5F_ACC_RDONLY_F, file, hdferr)
  ! ! Gestisco la memorizzazione della struttura: riapro il mio dataset dal nome "dataset" perchè devo copiarci alcuni valori da inserire in s2
  ...
  ! ! Create the memory data type: gestisce il dato e ne determina la natura:
  ! ! in questa fase sembra non esserci bisogno di un nuovo creatore di array (h5Tarray_create_f), ma perchè?
  !   CALL H5Tinsert_f(s2_tid, "chr", H5OFFSETOF(C_LOC(s2(1)),C_LOC(s2(1)%chr)), tid3, hdferr)
  ! CALL H5Tinsert_f(s2_tid, "c", H5OFFSETOF(C_LOC(s2(1)),C_LOC(s2(1)%c)), h5kind_to_type(r_k8,H5_REAL_KIND), hdferr)
  ! CALL H5Tinsert_f(s2_tid, "a", H5OFFSETOF(C_LOC(s2(1)),C_LOC(s2(1)%a)), h5kind_to_type(int_k1,H5_INTEGER_KIND), hdferr)
  ! ! idem: in questa fase sembra non esserci bisogno di un nuovo creatore di array (h5Tarray_create_f), ma perchè?
  !   CALL H5Tinsert_f(s2_tid, "d", H5OFFSETOF(C_LOC(s2(1)),C_LOC(s2(1)%d)), tid3a, hdferr)
  ! ! CALL H5Tinsert_f(s2_tid, "d", H5OFFSETOF(C_LOC(s2(1)),C_LOC(s2(1)%d%x)), tid3a, hdferr)
  ...
  ! ! Display the fields: qui potrei stampare anche i valori da s1, compaiono a schermo ugualmente
  ! DO i = 1, length
  ...
  ! DO i = 1, length
  !    WRITE(*,'(/,A,/,999(I0,1X))') "Field d :", s2(i)%d(:)
  ! ENDDO
  
  ! ! -----------------------------------------------------------------------------------------------------------------
  ! ! QUARTO STEP: LETTURA REALI
  ! ! -----------------------------------------------------------------------------------------------------------------
  
  ! ! Create a data type for s3.
  ! CALL H5Tcreate_f(H5T_COMPOUND_F, H5OFFSETOF(C_LOC(s3(1)),C_LOC(s3(2))),s3_tid, hdferr)
  ! CALL H5Tinsert_f(s3_tid, "b", 0_size_t, h5kind_to_type(r_k4,H5_REAL_KIND), hdferr)
