...
! FILES
  ...
  CHARACTER(LEN=*), PARAMETER :: H5FILE_NAME = "compund_array.h5"
...
! First derived-type and dataset
  TYPE s1_t
     REAL(KIND=r_k4), DIMENSION(13) :: chr
...
! Third "derived-type" (will be used to read float field of s1)
  ...
  CALL H5Tinsert_f(s1_tid, "chr_name", H5OFFSETOF(C_LOC(s1(1)),C_LOC(s1(1)%chr)), h5kind_to_type(r_k4,H5_REAL_KIND), hdferr)
  ...
  !
  DO i = 1, length
     WRITE(*,'(/,A,/,999(A,1X))') "Field chr :", s2(i)%chr(1:13)
  ENDDO
